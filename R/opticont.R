...
"asDefinite"<-function(K, quiet=FALSE){
  ...
  if(min(x)<0){
    if(!quiet){cat("Matrix has negative eigenvalues. Looking for approximate solution...\n")}
...
"help.opticont"<-function(K, phen){
  Traits  <- apply(phen[,c(-1,-2)],2,is.numeric)
  Traits  <- setdiff(names(Traits)[Traits], c("lb", "ub","oc"))
  cat("Available objective functions for opticont: \n")
  cat("     ", paste("min.", c(names(K), names(attr(K,"condProb"))), sep=""),"\n", sep="  ")
  cat("     ", paste("min.", Traits, sep="") ,"\n", sep="  ")
  cat("     ", paste("max.", Traits, sep="") ,"\n\n", sep="  ")
  cat("Available constraints for opticont: \n")
  cat("     ", paste("ub.", c(names(K), names(attr(K,"condProb"))), sep=""),"\n", sep="  ")
  cat("     ", paste("lb.", Traits, sep="") ,"\n", sep="  ")
  cat("     ", paste("ub.", Traits, sep="") ,"\n", sep="  ")
  cat("     ", paste("eq.", Traits, sep="") ,"\n", sep="  ")
  cat("       ub  lb\n\n")
  cat("Attention: Probably not all these objective functions and\n")
  cat("           constraints make sense in animal breeding\n\n")
}

"opticont"<-function(method=paste("min.", names(K)[1], sep=""), K, phen, con=list(), solver="cccp", quiet=FALSE, make.definite=solver=="csdp", ...){
  gc()
  ...
  Traits  <- apply(phen[,-1],2,is.numeric)
  Traits  <- setdiff(names(Traits)[Traits], c("lb", "ub","oc"))
  ...
  relax   <- length(quadcon)==0
  
  
  for(i in intersect(names(quadcon), names(cKin))){
    K[[i]] <- (1-quadcon[i])*K[[cKin[[i]][2]]] + K[[cKin[[i]][1]]]
    quadcon[i] <- 1
  }
  ...
  for(i in rownames(const)){
    if(const[i,"con"]=="ub" & !quiet)cat("  Mean ",const[i,"var"]," in the offspring is not exceeding ", const[i,"val"],".\n",sep="")
    if(const[i,"con"]=="lb" & !quiet)cat("  Mean ",const[i,"var"]," in the offspring is at least ", const[i,"val"],".\n",sep="")
    if(const[i,"con"]=="eq" & !quiet)cat("  Mean ",const[i,"var"]," in the offspring is equal to ", const[i,"val"],".\n",sep="")
  }
  for(i in setdiff(names(Res$quadcon), names(cKin))){
    if(!quiet)cat("  Mean kinship ", i, " in the offspring is not exceeding ", Res$quadcon[i],".\n",sep="")
  }
  for(i in intersect(names(Res$quadcon), names(cKin))){
    if(!quiet)cat("  Conditional kinship ", i, " in the offspring is not exceeding ", Res$quadcon[i],".\n",sep="")
  }
  
  if(length(lb)==2 & !is.null(names(lb))){
    if(!quiet)cat("  Minimum contribution of males to offspring is ",   lb[1],".\n",sep="")                
    if(!quiet)cat("  Minimum contribution of females to offspring is ", lb[2],".\n",sep="")
    lb<-lb[sex]
  } else{
    if(!quiet)cat("  Minimum contributions of animals to the offspring were provided.\n",sep="")
  }
    
  if(length(ub)==2 & !is.null(names(ub))){
    if(is.na(ub[1])){
      if(!quiet)cat("  Number of offspring of males is not limited.\n")              
      if(!quiet)cat("    (This implies that the maximum contribution per male to the offspring is 0.5).\n")              
    }else{
      if(ub[1]==-1){
        if(!quiet)cat("  All males have equal contributions to the offspring.\n")
        if(!quiet)cat("    (This implies that no optimization is done for the males).\n")                      
      }else{
        if(!quiet)cat("  Maximum contribution of males to offspring is ", ub[1],".\n",sep="")                
      }
    }
    if(is.na(ub[2])){
      if(!quiet)cat("  Number of offspring of femmales is not limited.\n")              
      if(!quiet)cat("    (This implies that the maximum contribution per female to the offspring is 0.5).\n")              
    }else{
      if(ub[2]==-1){
        if(!quiet)cat("  All females have equal contributions to the offspring.\n")
        if(!quiet)cat("    (This implies that no optimization is done for the females).\n")                      
      }else{
        if(!quiet)cat("  Maximum contribution of females to offspring is ", ub[2],".\n",sep="")
      }
    }
    ub<-ub[sex]
  ...
  if(nV==0){
    Res$parent$oc  <- lb
    kinNames <- setdiff(names(K),names(cKin))
...
#    }
    ...
    if(nC>0){
      ...
      CB <- t(cC)%*%as(K[[f1]][isC, isC],"matrix")%*%cC
      CN <- t(cC)%*%as(K[[f2]][isC, isC],"matrix")%*%cC
    }
  ...
  for(i in names(quadcon)){
    if(useChol){
      F[[i]] <- as(chol(asDefinite(K[[i]][isV, isV], quiet=quiet)),"matrix")
      g[[i]] <- rep(0,nV)
      if(nC>0){
