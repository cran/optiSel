...
-	[Population Specific Parameters](#population-specific-parameters)
    ...
    - [Native Genome Equivalent](#native-genome-equivalent)
...
- renames the first three columns as `Indiv`, `Sire`, `Dam`,
- replaces 0, "", and " " in columns `Sire` and `Dam` by `NA`,
- breaks loops and reports which animal was included in a loop,
- adds or updates column `Sex`. Sexes will be recoded as 1 for males and 2 for females.
...
The next step is to find out if the completeness of the pedigree is sufficient for the intended evaluation, and to identify individuals with insufficient pedigree information. This will be demonstrated at the example of a pedigree of Hinterwald cattle. 
```{r, results="hide"}
data(PedigWithErrors)
Pedig <- prePed(PedigWithErrors)
```

Function [completeness][completeness] can be used to determine the proportion of known ancestors of each individual of the actual breeding population in each ancestral generation:
...
The kinship between two individuals is the probability that two alleles randomly chosen from both individuals are IBD. A sparse matrix containing the kinship between all pairs of individuals can be computed with function [pedIBD][pedIBD]. It is half the numerator relationship matrix. The R code below computes the kinship between the female with ID `276000813677683` and all males from the actual breeding population that have a breeding value larger than 1.7 and a pedigree with at least 5 equivalent complete generations.
...
```
But the correlation is much smaller if all individuals are considered, which may lead to different selection decisions:
...
For this data set both is true: The average number of equivalent complete generations is only `r round(mean(Summary$equiGen),2)`, and it is unclear whether this number is low because pedigrees of indivduals from other breeds were cut, or because previously unregistered animals 
have been registered. Hence, pedigrees of introduced individuals from other breeds should not be cut to reduce this uncertainty.
...
Thus, the diversity at native alleles decreases as fast as in an idealized population consisting of `r  round(attributes(fD)$nativeNe,2)` individuals.
